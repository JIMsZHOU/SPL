function contract_satimage

	[train_y1, train_x] = libsvmread('satimage.scale');
    [test_y1, test_x] = libsvmread('satimage.scale.t');

    train_y=zeros(4435,6);
    for i = 1:1:4435
        for ii = 1:6
            if train_y1(i,:) == ii
                train_y(i,ii) =1;
	...
	cnum= floor(num/300); % SPLD cluster number
    cnum2 =floor(num/900); % NSPLD cluster number
